    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                f"SELECT id, device_address, illuminance, measure_time "
                f"FROM {C.TABLE} "
                f"WHERE measure_time BETWEEN %s AND %s "
...
def main_page(collector):
    ...
    if st.button("🔍 查询历史记录", type="primary"):
        ...
        if start_dt > end_dt:
            ...
            st.session_state["query_df"] = query_between(start_dt, end_dt)
            st.session_state["query_ok"] = True
            st.session_state["query_range"] = f"{start_dt:%Y-%m-%d %H:%M} ~ {end_dt:%Y-%m-%d %H:%M}"
    ...
    if st.session_state.get("query_ok"):
        ...
        e1, e2 = st.columns(2)
        e1.download_button(
            "⬇ 导出 CSV（文本文件）", csv_bytes,
            file_name=f"光照记录_{d1}_{d2}.csv", mime="text/csv",
            use_container_width=True)
        e2.download_button(
            "⬇ 导出 Excel（电子表格）", xlsx_buf.getvalue(),
